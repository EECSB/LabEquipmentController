...
public static class Commands
{
    ...
    /// <summary>
    /// The serial ports this machine has. The counterpart of <c>interfaces</c>: these are
    /// listed, never opened.
    ///
    /// Not the counterpart of <c>scan</c>, which the two UIs now have a serial half of
    /// (<see cref="SerialScanner"/>). That half exists because a window can put what it is
    /// about to do on the button and wait to be pressed; a command that opened every port on
    /// the machine the moment it was typed could not. What it would save here is one command:
    /// <c>lec id serial://COM3</c> asks the same question of the port you name, at whatever
    /// speed it is set to, and the names above are all that was missing.
    /// </summary>
    public static int Ports(ParsedCommand cmd, TextWriter stdout, TextWriter stderr)
    {
        var names = SerialPorts.Names();

        // An empty table is a true answer and an unhelpful one — it looks the same as a
        // command that did not run. The note goes to stderr so that --json and --csv stay
        // exactly what a pipe expects.
        if (names.Count == 0 && !cmd.Has("quiet"))
            stderr.Write("No serial ports on this machine. A USB-to-serial adapter appears as one when it is plugged in.\n");

        var rows = names
            .Select(p => (IReadOnlyList<string>)new[] { p, $"serial://{p}" })
            .ToList();
        return Emit(cmd, Output.Render(cmd, ["Port", "Address"], rows), stdout);
    }
    ...
    public static int Version(ParsedCommand cmd, TextWriter stdout)
    {
        ...
        var rows = new List<IReadOnlyList<string>>
        {
            new[] { "lec",      version?.ToString(3) ?? "unknown" },
