...
- **The binding strip is a table.** `SequenceForm` resolves a `DEVICE` line to a connected
  instrument by model and prints the answer as a line of text; there is nothing to choose. The web
  pre-binds the same way and puts a picker against each alias, for the case of two instruments of
  one model on the bench. The picker is the web-only part.
