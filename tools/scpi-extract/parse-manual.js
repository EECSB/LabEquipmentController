...
function parseRs() {
  ...
  // An indented heading wraps the same way — the FPC breaks
  //
  //   CALCulate<n>:LIMit<k>:DEFine <Name>, <Description>, <X-unit>, <X-scale>, <Y-
  //   Unit>, <X0>, <Y0>, <X1>, <Y1>[, <Xn>, <Yn>]
  //
  // and the truncated head shipped once, while the tail row stood as the entry's
  // description. The column-0 join above is keyed off the FSU's indentation, so the
  // indented case takes its own pass, and a narrower one: only a line whose first
  // token is a command and whose clause opens with a placeholder can be a wrapped
  // heading — prose and example rows carry neither — and it is joined only while
  // genuinely open, a comma, pipe or unclosed bracket at the margin. A break inside
  // a token — "<Y-" / "Unit>" — is glued without the space the margin never held.
  //
  // The continuation itself must read as arguments, tested with the head scan's own
  // prose words: the FSV's "MMEMory:STORe<n>:PEAK <FileName" is open too, and without
  // the gate the join swallowed the entry's description sentence by sentence — the
  // placeholder never closes — taking the neighbouring LIST? entry's prose with it.
  for (let i = 0; i < lines.length; i++) {
    if (!/^\s/.test(lines[i])) continue;
    const t = lines[i].trim();
    if (RS_LABEL.test(t)) continue;
    const head0 = t.split(/\s/)[0];
    const rest = t.slice(head0.length).trim();
    if (!rest || !/^[<{\[]/.test(rest) || !isCmd(head0)) continue;
    for (;;) {
      const unbal = (lines[i].match(/</g) || []).length !== (lines[i].match(/>/g) || []).length
                 || (lines[i].match(/\[/g) || []).length !== (lines[i].match(/\]/g) || []).length;
      if (!unbal && !/[,|]\s*$/.test(lines[i].trimEnd())) break;
      let n = i + 1;
      while (n < lines.length && !lines[n].trim()) n++;
      if (n >= lines.length) break;
      const nt = lines[n].trim();
      if (RS_LABEL.test(nt) || isCmd(nt)) break;
      if (/\b(the|and|for|with|this|that|see|use|command)\b/i.test(nt)) break;
      const midToken = /<[\w-]+$/.test(lines[i].trimEnd()) && /^[\w-]+>/.test(nt);
      lines[i] = lines[i].trimEnd() + (midToken ? '' : ' ') + nt;
      lines[n] = '';
    }
  }
