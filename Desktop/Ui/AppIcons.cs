...
internal static class AppIcons
{
    ...
    /// "ai", "save", "folder", "globe", "plus", "minus", "fit".
    /// </summary>
        ...
        try
        {
            ...
            using (var g = Graphics.FromImage(dst))
            {
                ...
                switch (name)
                {
                    ...
                    case "plus": DrawPlus(g); break;
                    case "minus": DrawMinus(g); break;
                    case "fit": DrawFit(g); break;
                    default: return null;
                }
            }
            _drawn[key] = dst;
    ...
    /// <summary>
    /// The three gestures a plot answers to, as marks: zoom in, zoom out, and show all of it.
    ///
    /// The same three the web build draws, coordinate for coordinate — see its Icon.razor. The
    /// plus and the minus share an arm so the pair reads as one control with two ends.
    /// </summary>
    private static void DrawPlus(Graphics g)
    {
        using Pen pen = Stroke(6f);
        g.DrawLine(pen, 32, 18, 32, 46);
        g.DrawLine(pen, 18, 32, 46, 32);
    }

    private static void DrawMinus(Graphics g)
    {
        using Pen pen = Stroke(6f);
        g.DrawLine(pen, 18, 32, 46, 32);
    }
    ...
    /// <summary>
    /// Two walls with an arrow spreading out to meet them: back to the whole record.
    ///
    /// Not the circular arrow Scan and Clear wear. That one means "do it again"; this one means
    /// "show all of it", and a plot that already has a Run button beside it does not need a
    /// second mark that looks like one.
    /// </summary>
    private static void DrawFit(Graphics g)
    {
        using Pen pen = Stroke(5f);
        g.DrawLine(pen, 13, 17, 13, 47);
        g.DrawLine(pen, 51, 17, 51, 47);
        g.DrawLine(pen, 23, 32, 41, 32);
        g.DrawLines(pen, [new PointF(27, 26), new PointF(21, 32), new PointF(27, 38)]);
        g.DrawLines(pen, [new PointF(37, 26), new PointF(43, 32), new PointF(37, 38)]);
    }
    ...
    /// <summary>
    /// A frame with a bold arrow leaving it, or the same frame with the arrow arriving: the
    /// console's window, and which way it is going.
    ///
    /// This was a bare diagonal arrow, and it was drawn that way on purpose — a box plus a
    /// shaft plus a head is a lot of ink for 16 device pixels, and an earlier attempt at the
    /// frame had the inbound arrow reading as a flag. Two things settled it the other way.
    /// The frame is what carries the meaning: the button says *where*, not which direction,
    /// and the label was doing all of that work alone. And the arrow alone made the pair
    /// near-identical — Detach and Re-attach were one shape mirrored, told apart only by
    /// their words.
    ///
    /// The flag was real, though, and the fix is that the inbound head is drawn well inside
    /// the frame rather than across the gap it enters by. On the corner it merges with it.
    /// </summary>
    private static void DrawMove(Graphics g, bool outward)
    {
        using var pen = new Pen(GlyphInk, 6f)
        {
            StartCap = LineCap.Round,
            EndCap = LineCap.Round,
            LineJoin = LineJoin.Round,
        ...
        // The frame: a rounded square open at its top-right corner, so the arrow passes
        // through the gap rather than crossing a line. Anticlockwise from the top edge —
        // AddArc draws from an angle for a sweep, measured clockwise from three o'clock,
        // so each corner is a quarter turn of -90 about its own centre.
        using (var frame = new GraphicsPath())
        {
            frame.AddLine(34, 14, 14, 14);
            frame.AddArc(10, 14, 8, 8, 270, -90);    // top-left
            frame.AddLine(10, 18, 10, 50);
            frame.AddArc(10, 46, 8, 8, 180, -90);    // bottom-left
            frame.AddLine(14, 54, 46, 54);
            frame.AddArc(42, 46, 8, 8, 90, -90);     // bottom-right
            frame.AddLine(50, 50, 50, 30);
            g.DrawPath(pen, frame);
        }

        if (outward)
        {
            // Out through the gap, head clear of the frame.
            g.DrawLine(pen, 32, 32, 45, 19);
            g.FillPolygon(fill, new[]
            {
                new PointF(58, 6), new PointF(50, 26), new PointF(38, 14),
            });
        }
        else
        {
            // In through the gap, head well inside the frame. Both heads are 17 units across
            // the base, so the pair weighs the same.
            g.DrawLine(pen, 58, 6, 36, 28);
            g.FillPolygon(fill, new[]
            {
                new PointF(22, 42), new PointF(29, 23), new PointF(41, 35),
            });
        }
    }
}
