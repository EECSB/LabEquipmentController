...
;
; Build the payload first (from the repository root; PublishDir is project-relative, so the
; output lands under Desktop\bin):
;   dotnet publish Desktop\LabEquipmentController.csproj -c Release -r win-x64 ^
...
;
; Verify the result with Desktop\installer\Test-Installer.ps1 — no elevation needed, because
; this installs per-user by default.
;
