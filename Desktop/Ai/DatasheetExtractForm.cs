...
namespace LabEquipmentController
{
    ...
    {
        // The connections to choose from, and their keys. A list rather than one, because the
        // model is a choice — the cheap fast one that suits reading commands out of a guide is
        // not the one you want writing a sequence — and the picker at the top is where that
        // choice is made while looking at the file it applies to.
        ...
        private string KeyFor(string id)
            => _keys.TryGetValue(id, out string? key) ? key : "";

        /// <summary>
        /// Spend a different connection, from here on.
        ///
        /// Written back to the settings rather than kept in this window: there is one selection
        /// for the app, so choosing here is choosing for the script writer too. An extraction
        /// already in flight keeps the connection it was started on — it was sent on that key.
        /// </summary>
        private void OnConnectionPicked(object? sender, EventArgs e)
        {
            if (_updating || _provider.SelectedIndex < 0) return;
            if (_provider.SelectedIndex >= _book.Items.Count) return;

            _connection = _book.Items[_provider.SelectedIndex].Clone();
        ...
        public DatasheetExtractForm(AiConnections book, IReadOnlyDictionary<string, string> keys,
        ...
        private void BuildUi()
        {
            ...
            // Which connection to spend, rather than a line saying which one it happens to be.
            // Choosing here chooses everywhere: there is one selection for the app, so this
            // window and the script writer cannot disagree about which key is being spent.
            ...
            _provider.Width = 320;
            _provider.Margin = new Padding(0, 4, 0, 0);
            _provider.SelectedIndexChanged += OnConnectionPicked;
        ...
        {
            // The list, with the labels Core composes — which is what keeps two connections on
            // one model apart here and in every other picker in both builds.
            if (_provider.Items.Count != _book.Items.Count)
            {
                _updating = true;
                try
                {
                    _provider.Items.Clear();
                    foreach (string label in _book.Labels()) _provider.Items.Add(label);
                    int at = _book.Items.FindIndex(c => c.Id == _connection.Id);
                    _provider.SelectedIndex = at >= 0 ? at : (_book.Items.Count > 0 ? 0 : -1);
                }
                finally { _updating = false; }
            }
