...
public sealed class InstrumentConsole : UserControl
{
    ...
    private void BuildUi()
    {
        ...
        _header = new TableLayoutPanel
        {
            ...
            ColumnCount = 1,
            RowCount = 1,
            // Generous bottom padding: this row says what the console is attached to, and the
            // strip below it sends commands to an instrument. Those are different jobs, and the
            // gap is what says so.
        ...
        // Nothing else up here. Detaching is a thing done *to* a tab, so it is on the tab —
        // beside the ✕, where a browser puts it and where the web build already has it. As a
        // labelled button on this line it cost a row's width to say what a glyph says, and it
        // could only be reached by first bringing the console to the front.
        ...
        // No ellipsis. The convention it belongs to is a command that stops to ask something
        // before it does anything — Open…, Save As…, Browse… — and this one does not ask: it
    ...
    {
        var buttons = new List<Button> { _btnSend, _btnClearLog, _btnSaveLog };
    ...
    /// <summary>Tell the console which way round it is being hosted.</summary>
    public void SetDetached(bool detached) => _detached = detached;
    ...
    private void OpenDatasheetExtract()
    {
        ...
        // window's own question — the picker at the top of it — so this only asks whether
        ...
        using var dlg = new DatasheetExtractForm(
            book, keys, string.IsNullOrEmpty(model) ? Session.Host : model, Session.Title);
    ...
    private async Task CaptureWaveformAsync()
    {
        ...
        try
        {
            ...
            // Channel 1, which is what a scope shows first. More of them is the window's
            // question: a second probe goes on the bench while the window is already open, so
            // the picker is in there and Run reads whatever is ticked when it comes round.
            IReadOnlyList<ChannelRead> wave = await ChannelCaptures.ReadAsync(
                Session.Client, Session.Profile.WaveformDialect, [1]);

            if (wave.Count == 0 || !wave[0].Ok) throw new InvalidOperationException(
                wave.Count > 0 && wave[0].Error is { Length: > 0 } why
                    ? "channel 1 " + why
                    : "the instrument returned no usable samples for channel 1");

            AppendLog($"--- waveform captured ({wave[0].Capture!.Samples.Count} points) ---",
                Color.MediumSpringGreen);

            // The window can ask for more of the same, and for other channels. It goes through
            // the session's client, so a running capture takes its turn on the link alongside
            // anything typed into the console rather than interleaving with it — see
            // SerializedInstrumentClient.
            var form = new WaveformForm(wave, Session.Title, RecaptureWaveformAsync);
    ...
    /// </summary>
    private async Task<IReadOnlyList<ChannelRead>> RecaptureWaveformAsync(
        IReadOnlyList<int> channels, CancellationToken ct)
    {
        if (!Session.IsConnected)
            throw new InvalidOperationException("the instrument is no longer connected");

        // Raised once around the lot rather than per channel: a full record is about a megabyte
        // and four of them are four.
        Session.Client.TimeoutMs = Math.Max(Session.UserTimeoutMs, 10000);
        try
        {
            return await ChannelCaptures.ReadAsync(
                Session.Client, Session.Profile.WaveformDialect, channels, ct);
        }
