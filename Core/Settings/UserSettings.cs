...
public sealed class UserSettings
{
    ...
    /// <summary>
    /// The user's AI connections, and which of them is in use.
    ///
    /// Null on a settings file written before there were several, which is what
    /// <see cref="AiBookOrMigrated"/> is for.
    /// </summary>
    public AiConnections? AiBook { get; set; }

    /// <summary>
    /// The one AI connection, as it was written before there were several.
    ///
    /// Read on the way in and never written again: <see cref="AiBookOrMigrated"/> folds it
    /// into the list as the first entry, keeping its key. Kept as a property rather than
    /// deleted so that reading an old file does not throw away what is in it — a user who
    /// set up a connection before this change should not have to set it up again.
    /// </summary>
    public AiConnection? Ai { get; set; }

    /// <summary>
    /// The connections, with a file written before this change folded in.
    ///
    /// Migration happens here rather than at load so that it is the same answer wherever it
    /// is asked from, and so that a file holding both shapes — which cannot happen but would
    /// be a settings file all the same — prefers the newer one.
    /// </summary>
    public AiConnections AiBookOrMigrated()
    {
        if (AiBook is { Items.Count: > 0 }) return AiBook;
        AiBook = AiConnections.From(Ai);
        return AiBook;
    }
    ...
    /// <summary>
    /// One encrypted key per connection, against <see cref="AiConnection.Id"/>.
    ///
    /// Keyed by id rather than by name or by position, because a connection can be renamed,
    /// re-pointed at another provider or moved up the list, and none of those should lose the
    /// key stored against it. Encrypted exactly as the single key above was, and for the same
    /// reason — see the remarks on it.
    /// </summary>
    public Dictionary<string, string>? AiApiKeysProtected { get; set; }
