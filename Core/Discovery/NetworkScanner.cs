...
public sealed class ScpiDevice
{
    ...
    /// <summary>Host and port, plainly. What the scan found, said as two numbers.</summary>
    public string Endpoint => $"{Address}:{Port}";

    /// <summary>
    /// The same instrument written the way it would be typed back in — <c>vxi://host</c> for
    /// VXI-11, <c>host:port</c> for a raw socket. This is what goes in an Address box when a
    /// row is picked, because <c>192.168.1.19:111</c> is the portmapper's port number and
    /// says nothing about the protocol behind it: it connects correctly (§5 reads 111 as
    /// VXI-11) and it reads as a raw socket on an odd port.
    ///
    /// One rule, in one place. The web had its own <c>d.Port == 111 ? …</c> and the desktop
    /// had none, which is how the desktop came to show the port and the web the protocol for
    /// the same row.
    /// </summary>
    public string TypedAddress =>
        new InstrumentAddress(Address.ToString(), Transport, Port, "inst0").Canonical;

    public string TransportName => Transport.DisplayName();
}
