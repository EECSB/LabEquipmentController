...
public sealed class InstrumentProfile
{
    ...
    public static InstrumentProfile ForIdentity(string? identity)
    {
        ...
        return FamilyForIdentity(identity) switch
        {
            ...
            InstrumentFamily.RohdeFsiqAnalyzer     => RohdeFsiqAnalyzer(model),
    ...
    public static InstrumentFamily FamilyForIdentity(string? identity)
    {
        ...
        // FSIQ before any looser FS test: it is the vector signal analyzer of the same
        // generation, with its own manual and its own catalog.
        if (rohde && m.StartsWith("FSIQ")) return InstrumentFamily.RohdeFsiqAnalyzer;
    ...
    private static bool IsSpectrumAnalyzer(string m)
        ...
        || m.StartsWith("MS2")
        // The FSE generation and the modern FSPN spectrum monitor: recognised as analyzers
        // so the R&S decline above catches them — no usable guide for either is here, and
        // an R&S analyzer with no catalog of its own must go Generic rather than fall
        // through to the Siglent set. FSIQ no longer reaches that decline — its own test
        // above claims it first, catalog and all — and stays listed only so the shape is
        // still read as an analyzer wherever this predicate is asked.
    ...
    /// <summary>
    /// R&amp;S FSIQ vector signal analyzers (FSIQ3, FSIQ7, FSIQ26, FSIQ40).
    ///
    /// Same generation as the FSU and FSQ and near enough the same command set, which is why
    /// the buttons match theirs — but it has its own manual and its own catalog, because the
    /// FSIQ is the vector analyzer of the family and carries a signal-analysis tree the
    /// spectrum analyzers do not. Sharing the FSQ's catalog would offer buttons for commands
    /// this instrument answers and hide the ones it has.
    /// </summary>
    private static InstrumentProfile RohdeFsiqAnalyzer(string model) => new()
    {
        Name = string.IsNullOrEmpty(model) ? "R&S FSIQ analyzer" : $"R&S FSIQ analyzer ({model})",
        ScreenCaptureSetup = new[]
        {
            "HCOPy:DEVice:LANGuage1 BMP",
            "HCOPy:DESTination1 'MMEM'",
