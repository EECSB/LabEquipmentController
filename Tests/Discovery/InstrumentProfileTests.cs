...
public class InstrumentProfileTests
{
    ...
    // Operating Manuals turned out to be on R&S's own CDNs and a mirror — and the FSIQ
    // followed when a copy of its Operating Manual was found, so it now has a catalog and
    // has moved out of this list.
    //
    // The FSE stays. Its manual was found too, but the copy is Volume 1, and chapter 6 —
    // the command reference — is in Volume 2. Extracting Volume 1 yields softkey prose and
    // page footers dressed as commands, so an FSEB30 still comes out Generic, and correctly:
    // the rule is that a *transcribed guide* earns a catalog, and half a guide is not one.
    // If someone ever widens a prefix test far enough to swallow it, this is what says so.

    [Theory]
    [InlineData("ROHDE&SCHWARZ,FSEB30,847121/004,3.30")]
    [InlineData("ROHDE&SCHWARZ,FSEM30,847121/004,3.30")]
