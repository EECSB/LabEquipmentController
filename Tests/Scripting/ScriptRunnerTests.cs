...
public class ScriptRunnerTests
{
    ...
    {
        // Every *IDN? reply has commas in it, so this is the common case, not an edge one.
        // Substituting before splitting turned this two-column row into five, and the CSV
        // that came out was misaligned from the first row down.
        var (_, rows) = await RunRecording(
            "COLUMNS Volts, Identity\n*IDN? -> who\nMEAS? -> v\nRECORD $v, $who",
            new Dictionary<string, string>
            {
                ["*IDN?"] = "ACME,FAKE-1000,SN000123,1.02",
                ["MEAS?"] = "1.234",
            });
        ...
        Assert.Equal("ACME,FAKE-1000,SN000123,1.02", row.Values[1]);
    }

    [Fact]
    public async Task A_reading_that_did_not_arrive_leaves_a_gap_rather_than_shifting_the_row()
    {
        // An empty capture used to be dropped, sliding every later value one column left —
        // which turns missing data into wrong data, silently.
        var (_, rows) = await RunRecording(
            "COLUMNS A, B, C\nONE? -> a\nTWO? -> b\nTHREE? -> c\nRECORD $a, $b, $c",
            new Dictionary<string, string> { ["ONE?"] = "1", ["TWO?"] = "", ["THREE?"] = "3" });
    ...
    [Fact]
    public async Task A_trailing_comma_is_punctuation_and_not_an_extra_column()
    {
        // Emptiness written in the script is a typo; emptiness produced by substitution is
        // a missing reading. Only the second one is kept.
        var (_, rows) = await RunRecording(
            "COLUMNS A, B\nONE? -> a\nRECORD $a, 42,",
            new Dictionary<string, string> { ["ONE?"] = "1" });

        var row = Assert.Single(rows);
        Assert.Equal(["1", "42"], row.Values);
    }

    [Fact]
    public async Task Ordinary_multi_argument_records_are_unchanged()
    {
        var (_, rows) = await RunRecording(
            "COLUMNS Freq, Vpp\nFREQ? -> f\nVPP? -> v\nRECORD $f, $v",
            new Dictionary<string, string> { ["FREQ?"] = "1000", ["VPP?"] = "2.5" });

        var row = Assert.Single(rows);
        Assert.Equal(["1000", "2.5"], row.Values);
    }
}
