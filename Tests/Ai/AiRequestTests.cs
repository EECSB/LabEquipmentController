...
public class AiRequestTests
{
    ...
    private static JsonNode Effortful(AiProvider p, AiEffort effort)
        => JsonNode.Parse(AiRequest.Body(
            new AiConnection { Provider = p, Effort = effort },
            "Extract the commands.", AiPayload.FromText("x"), Schema))!;

    ///
    ///The default is the absence of the field, not a value of it. It matters most on the
    ///OpenAI-compatible shape, which covers endpoints that reject a request carrying a
    ///parameter they have never heard of — so a user who has not asked for anything must not
    ///have anything asked on their behalf.
    ///
    [Theory]
    [InlineData(AiProvider.Gemini)]
    [InlineData(AiProvider.Anthropic)]
    [InlineData(AiProvider.OpenAiCompatible)]
    public void By_default_nothing_is_said_about_effort_at_all(AiProvider p)
    {
        JsonNode body = Effortful(p, AiEffort.Default);

        Assert.Null(body["generation_config"]);
        Assert.Null(body["reasoning_effort"]);
        Assert.Null(body["thinking"]);
    }

    ///
    ///Gemini keeps it in generation_config as thinking_level. Verified against the live API,
    ///which rejects unknown parameters inside that object as well as outside it — and answered
    ///the same question with 519 thought tokens at "low" against 3,030 at "high".
    ///
    [Theory]
    ...
    public void Gemini_asks_for_a_thinking_level(AiEffort effort, string expected)
    {
        JsonNode body = Effortful(AiProvider.Gemini, effort);

        Assert.Equal(expected, body["generation_config"]!["thinking_level"]!.GetValue<string>());
    }

    ///
    ///An OpenAI-compatible endpoint spells the same four the same way, at the top level.
    ///
    [Theory]
    [InlineData(AiEffort.Minimal, "minimal")]
    [InlineData(AiEffort.High, "high")]
    public void An_openai_endpoint_asks_for_a_reasoning_effort(AiEffort effort, string expected)
    {
        JsonNode body = Effortful(AiProvider.OpenAiCompatible, effort);

        Assert.Equal(expected, body["reasoning_effort"]!.GetValue<string>());
    }

    ///
    ///Anthropic buys it by the token, and the ceiling has to clear the budget with room left
    ///for an answer — a model that thinks up to its own max_tokens has nowhere to write.
    ///
    [Fact]
    public void Anthropic_buys_thinking_and_raises_the_ceiling_to_match()
    {
        JsonNode body = Effortful(AiProvider.Anthropic, AiEffort.High);

        Assert.Equal("enabled", body["thinking"]!["type"]!.GetValue<string>());
        int budget = body["thinking"]!["budget_tokens"]!.GetValue<int>();
        Assert.True(budget >= 1024, "Anthropic refuses a budget under 1,024 tokens.");
        Assert.True(body["max_tokens"]!.GetValue<int>() > budget,
                    "max_tokens must leave room for the reply as well as the thinking.");
    }

    ///
    ///There is no such thing as a little thinking on the Messages API — the floor is 1,024
    ///tokens — so the honest reading of "minimal" there is none, and none is no block.
    ///
    [Fact]
    public void Anthropic_reads_minimal_as_no_thinking_block()
    {
        JsonNode body = Effortful(AiProvider.Anthropic, AiEffort.Minimal);
