...
public class ScriptAuthorTests
{
    ...
    /// <summary>
    /// The window is a chat, and a chat whose history never reaches the model is a window
    /// showing the user something the model cannot see.
    /// </summary>
    [Fact]
    public void The_conversation_so_far_goes_across()
    {
        string payload = ScriptAuthor.BuildPayload(
            "now do the same at 5 Vpp", new[] { Gen() }, null, null, 240,
            new[] { new ScriptTurn("set a 1 kHz sine at 2 Vpp", "C1:BSWV WVTP,SINE", "Set it.") });

        Assert.Contains("conversation so far", payload);
        Assert.Contains("set a 1 kHz sine at 2 Vpp", payload);
        Assert.Contains("C1:BSWV WVTP,SINE", payload);
        Assert.Contains("Set it.", payload);
    }

    /// <summary>
    /// And a cleared conversation sends nothing, which is the whole reason Clear is there:
    /// a transcript that survived clearing would be a button that lies about the tokens.
    /// </summary>
    [Fact]
    public void A_cleared_conversation_carries_no_transcript()
    {
        Assert.DoesNotContain("conversation so far",
            ScriptAuthor.BuildPayload("write me a script", new[] { Gen() }, null, null));

        Assert.DoesNotContain("conversation so far",
            ScriptAuthor.BuildPayload("write me a script", new[] { Gen() }, null, null, 240, []));
    }

    /// <summary>
    /// The catalog check's finding travels with the turn it is about. A model told once that
    /// ":SOURce1:FREQuency" is in no catalog here, and then handed a transcript that omits
    /// the telling, writes it again — it has no other memory of the correction.
    /// </summary>
    [Fact]
    public void A_correction_from_an_earlier_turn_is_carried_back()
    {
        string payload = ScriptAuthor.BuildPayload(
            "try again", new[] { Gen() }, null, null, 240,
            new[]
            {
                new ScriptTurn("set the frequency", ":SOURce1:FREQuency 1000", "",
                               new[] { ":SOURce1:FREQuency 1000" }),
            });

        Assert.Contains("in no catalog on this bench", payload);
        Assert.Contains(":SOURce1:FREQuency 1000", payload);
    }

    /// <summary>
    /// The catalog trim is ranked on the request's own words, and a follow-up has none worth
    /// ranking on: "now do it twice as fast" names no instrument, no quantity and no command.
    /// Ranked on its own it takes 40 commands at random out of two thousand and throws away
    /// everything the conversation has been about.
    /// </summary>
    [Fact]
    public void A_follow_up_still_gets_the_commands_the_conversation_is_about()
    {
        ...
        var instrument = new ScriptContextInstrument(
            "", "FSL", "Rohde&Schwarz,FSL-6,1,1", analyzer);
        var history = new[]
        {
            new ScriptTurn("set the centre frequency and read the marker", ":FREQ:CENT 1e6"),
        };

        Assert.Contains("MARK", ScriptAuthor.BuildPayload(
            "now do it twice as fast", new[] { instrument }, null, null, 40, history));

        // Without the conversation, the same follow-up has nothing to rank on.
        Assert.DoesNotContain("MARK", ScriptAuthor.BuildPayload(
            "now do it twice as fast", new[] { instrument }, null, null, 40));
    }
