...
public class CliTests
{
    ...
    /// <summary>
    /// The line settings travel with the address, because a command line has one string and
    /// nothing else to put them in. An endpoint that dropped them would open COM3 at 9600
    /// against an instrument set to 115200 — which does not fail, it answers with rubbish.
    /// </summary>
    [Fact]
    public void A_serial_address_carries_its_line_settings()
    {
        Assert.True(Endpoint.TryParse("serial://COM3?baud=115200&parity=even", out var ep, out _));

        Assert.Equal(InstrumentTransport.Serial, ep.Transport);
        Assert.Equal("COM3", ep.Host);
        Assert.Equal(115200, ep.Serial!.BaudRate);
        Assert.Equal("COM3 (serial, 115200-8-E-1)", ep.ToString());
    }

    [Fact]
    public void A_serial_address_opens_a_serial_client()
    {
        Assert.True(Endpoint.TryParse("serial:///dev/ttyUSB0", out var ep, out _));

        using var client = ep.CreateClient(3000);
        Assert.Equal("/dev/ttyUSB0", client.Host);
        Assert.Contains("serial", client.Description);
    }

    /// <summary>
    /// Listed, never probed — the one thing 'ports' does not share with 'scan'. It has to
    /// run and produce a table on a machine with no serial ports at all, which is what CI is.
    /// </summary>
    [Fact]
    public void Ports_lists_what_there_is_without_opening_any_of_it()
    {
        var stdout = new StringWriter();
        var stderr = new StringWriter();
        int code = Commands.Ports(CommandLine.Parse(["ports"]), stdout, stderr);

        Assert.Equal(0, code);
        Assert.Contains("Port", stdout.ToString());
    }
